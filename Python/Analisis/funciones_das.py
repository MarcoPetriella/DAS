...
def peli_std(parametros):
    ...
    n = parametros['n']
    c_f = parametros['c_f']
    carpeta_std = parametros['carpeta_std']
    ...
    for file in sorted(os.listdir(os.path.join(time_str, carpeta_std)), reverse=True):
        print file
        if file.endswith(".std"):
            path_last_file = os.path.join(time_str, carpeta_std, file)
            last_file = int(file[:-4])
            break

    # Lista los archivos del directorio y se queda con el .std mas chico (eg. 000000.std)
    for file in sorted(os.listdir(os.path.join(time_str, carpeta_std)), reverse=False):
        print file
        if file.endswith(".std"):
            path_first_file = os.path.join(time_str, carpeta_std, file)
            break

    header_path = os.path.join(time_str, carpeta_std, 'std.hdr')
    header_path_avg = os.path.join(time_str, 'AVG', 'avg.hdr')
    direfig = os.path.join(time_str, carpeta_std, carpeta_figuras)
    ...
    def updatefig(j, offset_m, tiempo_actual):
        ...
        path = os.path.join(time_str, carpeta_std, file_num_str + '.std')
        path_avg = os.path.join(time_str, 'AVG', file_num_str + '.avg')
        ...
        if os.path.isdir(os.path.join(time_str, 'AVG')):
            header_avg, vector_avg = data_read(header_path_avg, path_avg, vec_fils, vec_cols) 
...
def carga_matriz_std(parametros):
    ...
    nombre_header = os.path.join(time_str, carpeta_std, 'std.hdr')
    nombre_archivo = os.path.join(time_str, carpeta_std, '000000.std')
    header = header_read(nombre_header, nombre_archivo)
    ...
    for i in range(ini_file, fin_file + 1):
        file_num = '%06d' % i
        # print file_num
        path = os.path.join(time_str, carpeta_std, file_num + '.std')
        header = header_read(nombre_header, path)
...
def carga_matriz_raw(parametros):
    '''
    Esta función carga la matriz STD (filas=tiempo, col=bines) de los archivos procesados .std del equipo de medición DAS.
    Aparte genera un grafico de los datos que guarda en un directorio nuevo llamado 'Figuras'.
    ...
    tiempo_ini = parametros['tiempo_ini']
    tiempo_fin = parametros['tiempo_fin']
    carpeta_raw = parametros['carpeta_raw']
    header_file = parametros['header_file']
    ...
    nombre_header = os.path.join(time_str, carpeta_raw, header_file)
    nombre_archivo = os.path.join(time_str, carpeta_raw, '000000.bin')
    header = header_read(nombre_header, nombre_archivo)

    sec_per_fila = 1. / FrecLaser
    filas = header['Fils']
    sec_per_file = filas * sec_per_fila
    ...
    for i in range(ini_file, fin_file + 1):
        file_num = '%06d' % i
        # print file_num
        path = os.path.join(time_str, carpeta_raw, file_num + '.bin')
        header = header_read(nombre_header, path)
...
#            print '%2.2f' % (float(j) / float(fin_file - ini_file) * 100.), ' %'
    ...
    for i in range(filas_tot):
        tt = tiempo_inii + datetime.timedelta(seconds=i * header['nShotsChk'] / FrecLaser)
        tiempo_filas_raw.append(tt)

    return matriz, tiempo_filas_raw, pos_bin,





def carga_matriz_avg(parametros):
    '''
    Esta función carga la matriz STD (filas=tiempo, col=bines) de los archivos procesados .std del equipo de medición DAS.
    Aparte genera un grafico de los datos que guarda en un directorio nuevo llamado 'Figuras'.
    ...
    nombre_header = os.path.join(time_str, carpeta_std, 'avg.hdr')
    nombre_archivo = os.path.join(time_str, carpeta_std, '000000.avg')
    header = header_read(nombre_header, nombre_archivo)
    ...
    for i in range(ini_file, fin_file + 1):
        file_num = '%06d' % i
        # print file_num
        path = os.path.join(time_str, carpeta_std, file_num + '.avg')
        header = header_read(nombre_header, path)
...
def carga_matriz_monitoreo(parametros):
    '''
    Esta función carga la matriz STD (filas=tiempo, col=bines) de los archivos procesados .std del equipo de medición DAS.
    Aparte genera un grafico de los datos que guarda en un directorio nuevo llamado 'Figuras'.
    ...
    time_str = parametros['time_str']
    FrecLaser = float(parametros['FrecLaser'])
    tiempo_ini = parametros['tiempo_ini']
    tiempo_fin = parametros['tiempo_fin']
    sub_dir = parametros['sub_dir']
    ...
    nombre_header = os.path.join(time_str, sub_dir, 'laser.mon.hdr')
    nombre_archivo = os.path.join(time_str, sub_dir, '000000.mon')
    header = header_read(nombre_header, nombre_archivo)

    nShotsChk = float(header['nShotsChk'])
    sec_per_fila = nShotsChk / FrecLaser
    filas = header['Fils']
    sec_per_file = filas * sec_per_fila
    ...
    matriz = np.zeros([filas_tot, int(header['Cols'])])
    vec_cols = np.array([0, header['Cols']-1], dtype=np.uint64)
    ind_fila = 0

    j = 0
    #print 'Cargando ',sub_dir,' : '
    for i in range(ini_file, fin_file + 1):
        file_num = '%06d' % i
        # print file_num
        path = os.path.join(time_str, sub_dir, file_num + '.mon')
        header = header_read(nombre_header, path)
    ...
    for i in range(filas_tot):
        tt = tiempo_inii + datetime.timedelta(seconds=i * header['nShotsChk'] / FrecLaser)
        tiempo_filas_mon.append(tt)

    return matriz, tiempo_filas_mon



def carga_datos_temperatura(parametros):
    ...
    data = np.loadtxt(file_name,delimiter=',', dtype=dt, unpack=True,skiprows=1)    
    ...
    for i in range(len(data[0])):
        ...
        if  (len(data[0][i]) > 19):
            tiempo_i = datetime.datetime.strptime(data[0][i], '%Y-%m-%d %H:%M:%S.%f')
        else:
            tiempo_i = datetime.datetime.strptime(data[0][i], '%Y-%m-%d %H:%M:%S')
        
        vector_tiempo.append(tiempo_i)
        
        delta_ti = tiempo_i - tiempo_ini_date
        delta_tf = tiempo_i - tiempo_fin_date
        
        if (abs(delta_ti.total_seconds())<dt1):
            row_i = i
            dt1 = abs(delta_ti.total_seconds())
    
        if (abs(delta_tf.total_seconds())<dt2):
            row_f = i
            dt2 = abs(delta_tf.total_seconds())


    matriz = []        
    for i in range(10):          
        matriz.append(data[i][row_i:row_f])
    
    vector_tiempo = vector_tiempo[row_i:row_f]     

    return matriz, vector_tiempo,



def carga_datos_temperatura2(parametros):
    ...
    nro_archivos = len(os.listdir(os.path.join(time_str, 'MONITOREO')))
    for t,file in enumerate(sorted(os.listdir(os.path.join(time_str, 'MONITOREO')), reverse=False)):
        
        if t == nro_archivos-2:
            break
        
        if file.endswith(".val"):
            path= os.path.join(time_str, 'MONITOREO', file)
            ...
            firstLine = infile.readline()
            tiempo_i = datetime.datetime.strptime(firstLine[0:26], '%Y-%m-%d %H:%M:%S.%f')
            
            
            deltai_ti = tiempo_ini_date - tiempo_i
            deltai_tf = tiempo_fin_date - tiempo_i
            
            deltai_ti = deltai_ti.total_seconds()
            deltai_tf = deltai_tf.total_seconds()
                        
            if deltai_ti > 0 and deltai_ti<delta_ti:
                delta_ti = deltai_ti
                path_first = path

            if deltai_tf > 0 and deltai_tf<delta_tf:
                delta_ti = deltai_ti
                path_last = path
        else:
            continue
    ...
    ind_first = int(path_first[len(path_first)-10:len(path_first)-4:])
    ind_last = int(path_last[len(path_last)-10:len(path_last)-4:])   
    
    data = np.loadtxt(path_first,delimiter=',', dtype=dt, unpack=True,skiprows=1)  
    vector_tiempo = []
    for i in range(len(data[0])):
        tiempo_i = datetime.datetime.strptime(data[0][i], '%Y-%m-%d %H:%M:%S.%f') 
        vector_tiempo.append(tiempo_i)
    ...
    for i,t in enumerate(vector_tiempo):
        if (t > tiempo_ini_date):
            ind_i = i
            break    

    vector_tiempo_f = []
    if path_first != path_last:
        data_f = np.loadtxt(path_last,delimiter=',', dtype=dt, unpack=True,skiprows=1)  
        for i in range(len(data_f[0])):
            tiempo_i = datetime.datetime.strptime(data_f[0][i], '%Y-%m-%d %H:%M:%S.%f')         
            vector_tiempo_f.append(tiempo_i)
    else:
        vector_tiempo_f = vector_tiempo

    ind_f = len(vector_tiempo_f)
    for i,t in enumerate(vector_tiempo_f):
        if (t > tiempo_fin_date):
            ind_f = i
            break           
    

    matriz = [] 
    path_b= os.path.join(time_str, 'MONITOREO')
    # cargo los datos
    if path_first == path_last:   
        for i in range(10):          
            matriz.append(data[i][ind_i:ind_f])
            
        vector_tiempo = vector_tiempo[ind_i:ind_f]    
    else:
        for i in range(10):          
            matriz.append(data[i][ind_i:len(data[0])])
                    
        vector_tiempo = vector_tiempo[ind_i:len(data[0])]  
        
        if ind_last - ind_first > 1:  
            for j in range(ind_first+1,ind_last):
                
                path_j = os.path.join(path_b, '%06d' % j + '.val' )
                data_j = np.loadtxt(path_j,delimiter=',', dtype=dt, unpack=True,skiprows=1)  
                
                for i in range(10):          
                    matriz[i] = np.append(matriz[i],data_j[i])            

                for i in range(len(data_j[0])):
                    tiempo_i = datetime.datetime.strptime(data_j[0][i], '%Y-%m-%d %H:%M:%S.%f') 
                    vector_tiempo.append(tiempo_i)
        
        for i in range(10):          
            matriz[i] = np.append(matriz[i],data_f[i][1:ind_f])            

        vector_tiempo.extend(vector_tiempo_f[1:ind_f])              
...
def carga_datos_temperatura3(parametros):
    ...
    tiempo_ini = parametros['tiempo_ini']
    tiempo_fin = parametros['tiempo_fin']
    carpeta_monitoreo = parametros['carpeta_monitoreo']
...
#    nro_archivos = len(os.listdir(os.path.join(time_str, 'MONITOREO')))
    for t,file in enumerate(sorted(os.listdir(os.path.join(time_str, carpeta_monitoreo)), reverse=False)):
        
#        if t == nro_archivos-2:
#            break
        
        if file.endswith(".val"):
            path= os.path.join(time_str, carpeta_monitoreo, file)
            ...
            if len(sep[0]) != 26:
                sep[0] = sep[0] + '.000000'
            tiempo_i = datetime.datetime.strptime(sep[0], '%Y-%m-%d %H:%M:%S.%f')
    ...
    if path_first != path_last:
        data_f = np.loadtxt(path_last,delimiter=',', dtype=dt, unpack=True,skiprows=1)  
        for i in range(len(data_f[0])):
            if len(data_f[0][i]) != 26:    
                data_f[0][i] = data_f[0][i] + '.000000'            
    ...
    matriz = [] 
    path_b= os.path.join(time_str, carpeta_monitoreo)
    # cargo los datos
    if path_first == path_last:   
        for i in range(19):          
            matriz.append(data[i][ind_i:ind_f])
        ...
        if ind_last - ind_first > 1:  
            for j in range(ind_first+1,ind_last):
                ...
                for i in range(len(data_j[0])):
                    if len(data_j[0][i]) != 26:    
                        data_j[0][i] = data_j[0][i] + '.000000'                          
