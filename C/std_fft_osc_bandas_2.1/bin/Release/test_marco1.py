...
def updatefig2(*args):
    ...
    if (ch_caxis==1):
        ...
        im.set_clim([float(mi_var),float(ma_var)])   
    
    # Plot
    mi_var_p = min_var_p.get()
    ma_var_p = max_var_p.get()
...
#  PERFIL
    ...
    ax1.set_xlim([0,tam])
    if (ch_caxis_p!=1):
        ax1.set_ylim([float(mi_var_p),float(ma_var_p)])   
    else:
...
def onclick(event):
    ...
    if (var_radio_b == 1):
        coord_1 = ix
        var1.set(round(coord_1))
    elif (var_radio_b == 2):
        coord_2 = ix
        var2.set(round(coord_2))
    elif (var_radio_b == 3):
        coord_3 = ix
        var3.set(round(coord_3))
    elif (var_radio_b == 4):
        coord_4 = ix
        var4.set(round(coord_4))
    elif (var_radio_b == 5):
        coord_5 = ix    
        var5.set(round(coord_5))
...
fig=Figure(figsize=(8, 6), dpi=125)
