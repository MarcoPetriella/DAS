...
void *procesa_STD(void* n){
    ...
    char command[100];
    //snprintf(command,100,"start cmd /k python test_marco1_m.py %d %d %d",perfil_mv_avg_size,window_bin,qFreq);
    snprintf(command,100,"start cmd /k python test_marco1_m.py %d %d %d",std_mv_avg_size,window_bin_std,qFreq);
    printf(command);
    system(command);
    ...
    //    printf("HOLA\n");
        ...
        for(i=1;i<nShotsChk-window_time+1;++i){
            ...
            for(j=0;j<window_bin;++j){
                mean_w[j] += -(float)(chunk_actual[(i-1)*bins*nCh + j*nCh]) + (float)(chunk_actual[(i+window_time-1)*bins*nCh + j*nCh]);
            ...
            for(j=1;j<bins-window_bin+1;++j){
                mean_w[j+window_bin-1] += -(float)(chunk_actual[(i-1)*bins*nCh + (j+window_bin-1)*nCh]) + (float)(chunk_actual[(i+window_time-1)*bins*nCh + (j+window_bin-1)*nCh]);
