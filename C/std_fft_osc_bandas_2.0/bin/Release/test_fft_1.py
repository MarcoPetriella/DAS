...
def updatefig2(*args):
    ...
    global ch1_r, ch2_r, ch3_r, ch4_r, ch5_r
    ...
    y_mi_var = float(y_min_var.get())
    y_ma_var = float(y_max_var.get())  
...
#    f=(args[0])       
    ...
    data = np.reshape(np.frombuffer(rd,dtype=np.float32) ,(bin1,filas))    
    data = np.transpose(data)
    
    #data=np.random.rand(bin1,filas)*i/100 
...
y_min = Tk.Entry(root, width=5, textvariable=y_min_var)
y_min.grid(column=2,row=6)
y_min_var.set(0)

y_max_var = Tk.StringVar()
y_max = Tk.Entry(root, width=5, textvariable=y_max_var)
y_max.grid(column=2,row=7)
y_max_var.set(np.round(filas))
...
label = Tk.Label(root,text="FFT ADQUISICION DAS").grid(column=0, row=0,columnspan=10)
fig=Figure(figsize=(10, 4), dpi=125)
...
ani = animation.FuncAnimation(fig, updatefig2, interval=1000, blit=False)
plt.show()
Tk.mainloop()
